...
class UtilizadorRepository(
    ...
    suspend fun getUtilizadores(token: String): List<Utilizador> {
        return try {
            val response = apiService.getUtilizadores("Bearer $token")
            if (response.isSuccessful) {
                response.body() ?: emptyList()
            } else {
                Log.e("UtilizadorRepository", "Erro ao obter utilizadores: ${response.code()}")
                emptyList()
            }
        } catch (e: Exception) {
            Log.e("UtilizadorRepository", "Falha na chamada para obter utilizadores", e)
            emptyList()
        }
    }

    /*
     * cria utilizador na API
     */
     ...
     */
    suspend fun apagarUtilizador(token: String, id: Int): Result<Unit> {
        return try {
            val response = apiService.deleteUtilizador("Bearer $token", id)
            if (response.isSuccessful) {
    ...
    suspend fun refreshUser(token: String, userId: Int): Result<Unit> {
        return try {
            ...
            if (response.isSuccessful) {
                response.body()?.let { utilizadorDaApi ->
